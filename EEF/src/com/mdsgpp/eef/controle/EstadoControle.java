...
public class EstadoControle {
	...
	public HashMap<String, String> getInformacoesEstado(int posicao, Context context) throws IOException {
		...
		informacoes.put("ideb_fundamental_final", "Ensino Fundamental (séries finais): "+ df.format( estado.getIdebs()[estado.getIdebs().length-1].getFundamental()) );
		informacoes.put("ideb_ensino_medio", "Ensino Médio: " +df.format(  estado.getIdebs()[estado.getIdebs().length-1].getMedio() ) + "" );
		informacoes.put("ideb_fundamental_inicial", "Ensino Fundamental (Séries iniciais): " + df.format( estado.getIdebs()[estado.getIdebs().length-1].getSeriesIniciais() ) + "");
