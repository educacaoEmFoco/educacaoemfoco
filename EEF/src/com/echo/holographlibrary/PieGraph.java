...
public class PieGraph extends View {
	...
	public void onDraw(Canvas canvas) {
		...
		for(PieSlice slice : slices) {
			...
			p.arcTo(new RectF(midX - radius, midY - radius, midX + radius,
				midY + radius), currentAngle + padding, currentSweep - padding);
			...
			if(indexSelected == count && listener != null) {
				...
				if(slices.size() > 1) {
					path.arcTo(new RectF(midX - radius - (padding * 2),
						midY - radius - (padding * 2),
						midX + radius + (padding * 2),
						midY + radius + (padding * 2)), currentAngle,
						currentSweep + padding);
